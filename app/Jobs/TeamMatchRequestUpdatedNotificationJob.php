...
class TeamMatchRequestUpdatedNotificationJob implements ShouldQueue
{
    use Dispatchable;
    use Queueable;
    ...
    public function handle(ExpoPushService $expoPushService): void
    {
        ...
        $message = $dateLabel !== ''
            ? sprintf('%s a modifié la demande de match (le %s)', $homeName, $dateLabel)
            : sprintf('%s a modifié la demande de match', $homeName);

        $this->notifyTeamRoster(
            $expoPushService,
            (int) $match->away_team_id,
            $this->actorUserId,
            'match_invite_updated',
            $message,
            $homeName,
            $this->matchEventId,
