...
=== .ai/ecran de connection et d'inscription rules ===
...
=== .ai/ecran de creation d'equipe rules ===
...
=== .ai/ecran de gestion de paiement rules ===
...
=== .ai/ecran de gestion de parametres rules ===
...
=== .ai/ecran de gestion des match - gestion de du profil d'equipe rules ===
...
=== .ai/ecran de moteur de recherche rules ===
...
=== .ai/ecran de notifications rules ===
...
=== .ai/ecran forfait de paiement rules ===
...
=== .ai/ecran post publication rules ===
...
=== .ai/ecran profil utilisateur rules ===
...
=== .ai/les premier ecran de direction rules ===
...
=== .ai/osport-design-complet-google-stitch rules ===
...
=== .ai/osport-guide-agents-synthese-design rules ===
...
=== .ai/schema-mysql-laravel12-osport rules ===
...
## O’Sport — règles projet (dossier `.ai/guidelines`)

Ces règles s’appliquent **en plus** du bloc Laravel Boost ci-dessus. Pour limiter la **dette technique**, les traiter comme **contraintes** : si une demande utilisateur les contredit, **refuser ou proposer une alternative alignée**, et ne déroger qu’après **accord explicite** de l’équipe (issue / message clair).

### Quand les charger (obligatoire)

- **Backend, API, persistance, schéma SQL, perf données** : ouvrir (ou garder en contexte) `.ai/guidelines/osport-guide-agents-synthese-design.md` **et** `.ai/guidelines/schema-mysql-laravel12-osport.md` — au minimum **Orientation**, **Cohérence d’abord**, **§1.7** du schéma.
- **UX, flux, libellés, maquettes** : ouvrir le `.ai/guidelines/ecran …` pertinent **et/ou** `.ai/guidelines/osport-design-complet-google-stitch.md`.

### Règles à ne pas casser (résumé)

- **§1.7 schéma** : Query Builder pour le relationnel métier ; Eloquent **sans** graphe lourd (`with()`, navigation relationnelle) sur ces chemins — les API Resources servent à **formater** la réponse, pas à charger toute la graphe en Eloquent.
- **Guide agents** : contrôleurs **≤ 100 lignes** ; Form Requests ; policies ; une seule source de vérité (prix, plans, billing) ; pas d’endpoint dupliqué pour le même état métier.
- **Cohérence d’abord** : aligner tout nouveau code sur les **fichiers voisins** et ces guidelines avant d’introduire un second style.

### Index rapide

| Fichier | Usage |
|---------|--------|
| `.ai/guidelines/osport-guide-agents-synthese-design.md` | Backend Laravel, patterns, domaines |
| `.ai/guidelines/schema-mysql-laravel12-osport.md` | Migrations, tables, index, §1.7 |
| `.ai/guidelines/osport-design-complet-google-stitch.md` | Agrégat UX |
| `.ai/guidelines/ecran … .md` | Spec détaillée par flux |
